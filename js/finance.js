...
import { ringChart, ringLegend, barChart, gauge, lineChart } from './charts.js';
...
/* Прежние donutSVG / sparkSVG / barsHTML заменены модулем charts.js */
...
function renderDash(box) {
  ...
  <section class="fin-hero glass">
    ...
    <div class="fh-right">
      ...
      <div class="ch-host" data-ch="trend"></div>
    </div>
  </section>
  ...
  <section class="fin-stats">
    <div class="fs-card glass fs-save">
      ...
      <div class="ch-host ch-gauge-host" data-ch="savings"></div>
  ...
  <div class="fin-grid">
    <section class="fin-card glass">
      ...
      ${donutParts.length ? `<div class="fc-ring-row">
        <div class="ch-host" data-ch="cats"></div>
        <div class="ch-legend-host" data-leg="cats"></div>
      </div>` : '<p class="empty">Расходов пока нет</p>'}
    </section>

    <section class="fin-card glass">
      <header class="fc-head"><h4>Расходы по месяцам · ${year}</h4><span class="fc-sub">клик — открыть месяц</span></header>
      <div class="ch-host" data-ch="year"></div>
    </section>

    <section class="fin-card glass">
      <header class="fc-head"><h4>Состав активов</h4><span class="fc-sub">${moneyShort(t.assets)}</span></header>
      ${accParts.length ? `<div class="fc-ring-row">
        <div class="ch-host" data-ch="assets"></div>
        <div class="ch-legend-host" data-leg="assets"></div>
      </div>` : '<p class="empty">Добавьте счета на вкладке «Счета»</p>'}
    </section>

    <section class="fin-card glass">
      <header class="fc-head"><h4>Последние 6 месяцев</h4></header>
      <div class="ch-host" data-ch="six"></div>
    </section>
  ...
  mountDashCharts(box, { s, t, trend, donutParts, accParts, six, heat });
...
/* ── Графики обзора ──
   Разметка строится строкой, а графикам нужны реальные размеры
   контейнера (ширина для столбиков и линии), поэтому монтируем их
   после того, как HTML уже в документе. */
function mountDashCharts(box, d) {
  const q = sel => box.querySelector(sel);
  const trendEl = q('[data-ch="trend"]');
  if (trendEl) {
    const dim = new Date(+curM.slice(0, 4), +curM.slice(5, 7), 0).getDate();
    lineChart(trendEl, d.trend.length > 1 ? d.trend : [0, 0],
      Array.from({ length: dim }, (_, i) => `${i + 1} ${MONTHS[+curM.slice(5, 7) - 1]}`),
      { height: 118, format: v => money(v), color: d.s.balance < 0 ? 'var(--c-skipped)' : 'var(--chart-1)' });
  }
  const sv = q('[data-ch="savings"]');
  if (sv) {
    gauge(sv, Math.max(0, Math.min(100, d.s.savings)), {
      size: 132, notches: 32,
      center: `${d.s.savings.toFixed(1)}%`, label: 'от дохода',
      color: d.s.savings < 0 ? 'var(--c-skipped)' : 'var(--chart-1)',
    });
  }
  const mountRing = (key, parts, total, label) => {
    const el = q(`[data-ch="${key}"]`);
    const leg = q(`[data-leg="${key}"]`);
    if (!el) return;
    const ring = ringChart(el, parts.map(p => ({
      label: p.name, value: p.sum, maxValue: total || 1, color: p.color,
    })), { size: 168, label, total, format: v => moneyShort(v) });
    if (leg) ringLegend(leg, ring.rings, v => money(v), i => ring.setHover(i));
  };
  mountRing('cats', d.donutParts, d.s.expense, 'Расход');
  mountRing('assets', d.accParts, d.t.assets, 'Активы');
  const yr = q('[data-ch="year"]');
  if (yr) {
    barChart(yr, d.heat.map(h => ({ x: MONTHS[h.i - 1], expense: h.sum })), [
      { key: 'expense', label: 'Расход', color: 'var(--chart-1)' },
    ], {
      height: 170, format: v => money(v),
      active: d.heat.findIndex(h => h.m === curM),
      onPick: i => { curM = d.heat[i].m; rerender(); },
    });
  }
  const six = q('[data-ch="six"]');
  if (six) {
    barChart(six, d.six.map(r => ({ x: monthShort(r.m), expense: r.expense, income: r.income })), [
      { key: 'income', label: 'Доход', color: 'var(--chart-2)' },
      { key: 'expense', label: 'Расход', color: 'var(--chart-5)' },
    ], { height: 178, format: v => money(v) });
  }
}
...
export function renderFinanceWidget(box) {
  ...
  box.innerHTML = `
    ...
    <div class="fw-main">
      <div class="ch-host fw-ring" data-ch="fw"></div>
      <div class="fw-lines">
  ...
  const fw = box.querySelector('[data-ch="fw"]');
  if (fw) {
    ringChart(fw, parts.map(p => ({ label: p.name, value: p.sum, maxValue: s.expense || 1, color: p.color })),
      { size: 104, stroke: 8, gap: 3, inner: 18, label: 'Расход', total: s.expense, format: v => moneyShort(v) });
  }

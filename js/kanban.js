...
import { renderTimeline, refreshTimeline, setTimelineRange, timelineRange, shiftTimeline, toggleExpanded, timelineExpanded, setTimelineHooks, TL_RANGES } from './timeline.js';
...
/* Шапку таймлайна собирает сам модуль — мы лишь отдаём ему обработчики
   и общее троеточие. Так они переживают любую перерисовку панели. */
setTimelineHooks({
  viewBtn: () => ensureViewBtn(),
  onTitle: anchor => { closeAllMenus(); openViewMenu(anchor); },
  onRange: anchor => { closeAllMenus(); openTimelineMenu(anchor); },
  onStep: dir => {
